...
`evals/run.sh` runs the skills on fixtures with planted problems in a fresh Claude Code session and checks the result mechanically: behaviour test still passes, every planted pattern gone, file under a line ceiling, one commit per slice for `/deslop repo`, for `/audit` the check restores the pre-fix code and requires the new tests to fail, for `/test-audit` the check applies three mutants and fails unless the suite kills all three, and for `ts-clean` correct code must come back byte-identical. Every run records tokens, cost, turns, time, and the Claude Code version in `evals/results/log.tsv`; `--baseline` runs the same fixture with no skills and a plain-English prompt, so the comparison is against the same model under the same conditions. Seven fixtures today. The result that matters most is the one against the toolkit: on the three fixtures with a baseline arm, the bare model with a one-sentence prompt passed the same checks at a third to a quarter of the cost. The fixtures are small and the problems are planted, so they catch regressions in the skills and put a number on their cost; they do not show that the skills produce better code than the same model without them. That claim needs harder fixtures, which do not exist here yet. [evals/README.md](./evals/README.md) has every row and the limits; [docs/COMPARISON.md](./docs/COMPARISON.md) sets this repo against the alternatives people install, including where it is weaker.

## Status

| | Implemented | Tested | Intended, not yet shown |
|---|---|---|---|
| Writing without slop (`clean-code`) | rules, gates, report | fixtures show the cleanup passes remove the planted patterns; the baseline arm shows the bare model removes them too | that a model with the skill writes less slop than without it over a real feature |
...
| `/audit` | catalog, four facts per finding, red-first fixes | `ts-audit` with a mechanical red-first check; it also found a real bug in `ts-clean`. The bare model passed the same check at lower cost | precision and recall on bugs nobody planted; any advantage over the bare model |
| `/test-audit` | classification, seam tests, mutation probes | `ts-test-audit` with mutants applied by the check. The bare model passed the same check at lower cost | results with a real mutation tool; any advantage over the bare model |
