...
- Measured on release day (Claude Code 2.1.258, rows in `evals/results/log.tsv`): skills arm PASS on `ts-clean` (211s, $1.92), `ts-audit` (145s, $1.42), `ts-test-audit` (228s, $1.76); baseline arm, no skills, two runs each, PASS on all three at $0.51 to $1.07. On these fixtures the bare model matches the skills' outcome at a third to a quarter of the cost; the skills add process (reports, `Unverified`, findings with severity and confidence), not a measured outcome advantage. Harder fixtures are the next step.
